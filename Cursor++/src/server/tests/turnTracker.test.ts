...
describe('turnTracker', () => {
  it('materializes new turns and resumes from cached turn blobs', () => {
    ...
    store.cacheBlob(userBlob.blobId, userBlob.blobData)
    ...
    if (thinking)
      store.cacheBlob(thinking.blobId, thinking.blobData)
    if (assistant)
      store.cacheBlob(assistant.blobId, assistant.blobData)

    const turnBlob = turn.materializeTurnBlob()
    store.cacheBlob(turnBlob.blobId, turnBlob.blobData)
